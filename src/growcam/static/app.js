...
const liveQualityButtons = [
    ...document.querySelectorAll("[data-live-quality]"),
];
const cameraControlRetry = document.querySelector("#camera-control-retry");
const tabButtons = [...document.querySelectorAll(".app-tab")];
// eslint-disable-next-line perfectionist/sort-sets -- Keep the tested protected-tab feature flow order.
...
let isFormIsDirty = false;
let isAppSettingsDirty = false;
let currentPreviewRecording = null;
let previewRequest = null;
let historyRequest = null;
let isAppReady = false;
let isHistoryLoaded = false;
let isTimelapseLoaded = false;
let isFilesLoaded = false;
let historyLoadPromise = null;
let timelapseLoadPromise = null;
let filesLoadPromise = null;
// eslint-disable-next-line unicorn/consistent-boolean-name -- Preserve the browser source contract asserted by server tests.
...
const isNativeHevcSupported = browserSupportsNativeHevc();
...
function displayDate(value) {
    if (!value) return "—";
    return cameraDate(value).toLocaleString([], {
        dateStyle: "medium",
        timeStyle: "short",
    });
}
...
function setIconLabel(element, icon, label) {
    const iconElement = document.createElement("span");
    iconElement.className = "nf";
    iconElement.setAttribute("aria-hidden", "true");
    iconElement.textContent = icon;
    element.replaceChildren(iconElement, document.createTextNode(` ${label}`));
}

function storedLiveQuality() {
    try {
        return window.localStorage.getItem("growcam-live-quality") === "sd"
            ? "sd"
            : "fhd";
    } catch {
        return "fhd";
...
function updateLiveQualityButtons(isDisabled = false) {
    for (const button of liveQualityButtons) {
        const isSelected = button.dataset.liveQuality === liveQuality;
...
function loadVideoStream(video, url, signal) {
    return new Promise((resolve, reject) => {
        ...
        const handleError = () => {
            removeAllListeners();
            reject(
                new Error("The browser could not start this camera preview.")
            );
        };
        const handleAbort = () => {
            removeAllListeners();
            stopMediaLoad(video);
            reject(
                new DOMException("The media load was aborted.", "AbortError")
            );
        };
...
async function previewCacheState(url, signal) {
    const probeUrl = new URL(url, location.href);
    ...
    if (!response.ok)
        throw new Error(`Preview cache check failed (${response.status}).`);
    const payload = await response.json();
    return payload.building ? "building" : "missing";
}

async function promoteCachedVideo(
    video,
    url,
    request,
    isCurrent,
...
async function resumeCachedVideo(
    video,
    url,
    request,
    isCurrent,
    ...
    if (shouldResume) await video.play().catch(() => undefined);
    onReady();
}
...
function renderCameraInfo(info) {
    ...
    document.querySelector("#device").textContent =
        `${info.login?.device_type || "IPC"} · ${system.DeviceModel || "GrowCam"}`;
    document.querySelector("#firmware").textContent =
        system.SoftWareVersion || "Firmware unknown";
    document.querySelector("#storage-total").textContent = formatSize(
        totalMib * 1024 ** 2
    );
    document.querySelector("#storage-free").textContent = formatSize(
        freeMib * 1024 ** 2
    );
    document.querySelector("#storage-percent").textContent = totalMib
        ? `${((freeMib / totalMib) * 100).toFixed(1)}% available`
        : "Capacity unavailable";
    document.querySelector("#storage-window").textContent =
        `${partition.OldStartTime || "—"} → ${partition.NewEndTime || "—"}`;
    document.querySelector("#timelapse-storage-estimate").textContent =
        formatSize(estimatedTimelapseMib * 1024 ** 2);
    ...
    cameraControlState = info.cameraControl || {
        available: true,
        status: "available",
    };
...
function cameraControlStateFromError(error) {
    const state =
        error instanceof HttpError ? error.payload?.cameraControl : null;
...
function updateCameraControlTabIndicators(unavailable) {
    for (const button of tabButtons) {
        ...
        if (unavailable) {
            button.title =
                "Camera controls are paused; this tab will not contact the camera.";
        } else {
...
function updateCameraControlRetry() {
    const isLocked = cameraControlState?.locked === true;
    const isRetryAllowed = !isLocked && cameraControlState?.retryAllowed === true;
    const isRetryExhausted =
        cameraControlState?.circuitOpen === true && !isLocked && !isRetryAllowed;
    cameraControlRetry.disabled = !isRetryAllowed;
    let buttonIcon = "󰑓";
    let buttonLabel = "Retry camera controls";
    if (isLocked) {
        buttonIcon = "";
        buttonLabel = "Retry disabled after Ret=205";
    } else if (isRetryExhausted) {
        buttonIcon = "󰌾";
        buttonLabel = "Retry disabled for this run";
    }
    setIconLabel(cameraControlRetry, buttonIcon, buttonLabel);
    if (isLocked) {
        cameraControlRetry.title =
            "This server will not retry a locked camera account. Restart GrowCam after unlocking it.";
    } else if (isRetryExhausted) {
        cameraControlRetry.title =
            "GrowCam will not make another login attempt in this server run.";
    } else {
...
function renderProtectedTabUnavailable(name) {
    let guidance =
        "Camera controls are paused. Use Retry camera controls above when you are ready.";
    if (cameraControlState?.locked) {
        guidance =
            "Camera reported an account lock. Unlock or reset it, then restart GrowCam.";
    } else if (cameraControlState?.retryAllowed === false) {
        guidance =
            "No more logins will be attempted in this server run. Verify access, then restart GrowCam.";
    }
    if (name === "rewind") {
        document.querySelector("#history-status").textContent = guidance;
    } else if (name === "timelapse") {
        document
            .querySelector("#panel-timelapse")
            .setAttribute("aria-busy", "false");
        setStatus(
            document.querySelector("#timelapse-state"),
            "Controls paused",
            "warning"
        );
...
function renderCameraControlUnavailable(error) {
    ...
    document.querySelector("#storage-percent").textContent =
        "DVRIP access required";
    document.querySelector("#storage-window").textContent =
        "DVRIP access required";
    document.querySelector("#timelapse-storage-estimate").textContent =
        "Unavailable";
    document.querySelector("#timelapse-storage-note").textContent =
        "DVRIP access required";

    const note = document.querySelector("#camera-control-note");
    let detail = String(
        cameraControlState.message || errorMessage(error)
    ).trimEnd();
    while (detail.endsWith(".")) detail = detail.slice(0, -1);
    let nextStep =
        "Automatic DVRIP attempts are paused. Use the explicit retry button when you want to try once more.";
    if (cameraControlState.locked) {
        nextStep =
            "This server will not attempt another login after Ret=205. Unlock or reset the camera, then restart GrowCam.";
    } else if (cameraControlState.retryAllowed === false) {
        nextStep =
            "This server will not make another login attempt. Verify camera access, then restart GrowCam.";
    }
    document.querySelector("#camera-control-error").textContent =
        `${detail}. ${nextStep} Live video remains independent over RTSP.`;
...
function clockForSeconds(value) {
    const safeSeconds = Math.max(0, Math.min(86_399, Math.round(value)));
...
function recordingBounds(recording) {
    ...
    const end = endDate > beginDate ? 86_400 : Math.max(start, rawEnd);
    return { end, start };
}
...
function nextHistoryPointAfter(index, seconds) {
    ...
    for (let nextIndex = index; nextIndex < recordings.length; nextIndex += 1) {
        ...
        if (seconds < bounds.end) {
            return {
                index: nextIndex,
                seconds: Math.max(seconds, bounds.start),
            };
        }
    }
    return null;
...
function historyWindowLabel(availableSeconds = null) {
    ...
    if (previewSeconds < 60)
        return `${Math.round(previewSeconds)}-second preview`;
    return `${previewSeconds / 60}-minute preview`;
}

function setScrubberPosition(seconds) {
    const safeSeconds = Math.max(0, Math.min(86_399, Math.round(seconds)));
    historyScrubber.value = String(safeSeconds);
    document.querySelector("#history-scrubber-time").textContent =
        clockForSeconds(safeSeconds);
}

function highlightHistorySelection(index) {
    for (const button of document.querySelectorAll(
        ".timeline-segment, .segment-button"
    )) {
...
function activateTab(name, shouldFocus = false, shouldReveal = false) {
    const selectedButton =
        tabButtons.find((button) => button.dataset.tab === name) ||
        tabButtons[0];
    for (const button of tabButtons) {
    ...
    url.hash =
        selectedButton.dataset.tab === "live" ? "" : selectedButton.dataset.tab;
    ...
    if (isAppReady) void ensureTabData(selectedTab);
}
...
function startLiveFeed() {
    if (livePaused || document.querySelector("#panel-live").hidden) return;
    if (liveFeed.hasAttribute("src")) return;
    liveMessage.textContent = `Starting ${liveQuality.toUpperCase()} live video…`;
    document.querySelector("#live-resolution").textContent =
        `${liveQuality.toUpperCase()} · connecting`;
...
function updateLivePauseState() {
    ...
    document
        .querySelector("#live-state-label")
        .classList.toggle("paused", livePaused);
    document.querySelector("#live-state-text").textContent = livePaused
        ? "PAUSED"
        : "LIVE";
...
function stopLiveFeed(message) {
    ...
    document.querySelector("#live-resolution").textContent =
        `${liveQuality.toUpperCase()} · paused`;
    livePlaceholder.hidden = false;
    updateLiveQualityButtons(false);
}
...
function selectLiveQuality(quality) {
    if (quality === liveQuality || !["fhd", "sd"].includes(quality)) return;
    liveQuality = quality;
    rememberLiveQuality();
    updateLiveQualityButtons(true);
    restartLiveFeed();
...
function updateLiveAudioState(active, label) {
    ...
    setIconLabel(button, "󰝟", active ? "Disable audio" : "Enable audio");
...
async function toggleLiveAudio() {
    ...
    try {
        ...
        stopLiveAudio();
        document.querySelector("#live-audio-state").textContent =
            `· ${errorMessage(error)}`;
    }
}
...
function tabDataLoader(name) {
    if (name === "rewind" && !isHistoryLoaded) return loadHistory;
    if (name === "timelapse" && !isTimelapseLoaded) return loadTimelapse;
    if (name === "files" && !isFilesLoaded) return loadFiles;
...
function renderTabDataError(name, error) {
    ...
    if (name === "rewind") {
        document.querySelector("#history-status").textContent = message;
    } else if (name === "timelapse") {
        setStatus(
            document.querySelector("#timelapse-state"),
            "Camera unavailable",
        ...
        document.querySelector("#files-status").textContent = message;
    } else if (name === "settings") {
        setStatus(
            document.querySelector("#app-settings-state"),
...
function renderCacheStats(cache) {
    ...
    document.querySelector("#cache-used").textContent =
        formatCacheSize(currentBytes);
    document.querySelector("#cache-limit").textContent =
        `of ${formatSize(maximumBytes)}`;
    ...
    document.querySelector("#show-clear-cache").disabled =
        cache.busy || cache.entryCount === 0;
}

/**
 *
 */
function renderAppSettings(payload, shouldForceFormRefresh = false) {
    appSettingsData = payload;
    const settings = payload.settings;
    if (!isAppSettingsDirty || shouldForceFormRefresh) {
        const gibibytes = settings.cacheMaxBytes / 1024 ** 3;
        document.querySelector("#cache-size-gib").value = String(
            Number(gibibytes.toFixed(3))
        );
        document.querySelector("#cache-max-entries").value = String(
            settings.cacheMaxEntries
        );
        document.querySelector("#rewind-window-setting").value = String(
            settings.rewindPreviewSeconds
        );
        document.querySelector("#continue-playback-setting").checked =
            settings.continuePlayback;
        document.querySelector("#preview-video-codec-setting").value =
            settings.previewVideoCodec;
        isAppSettingsDirty = false;
    }
    historyWindow.value = String(settings.rewindPreviewSeconds);
    document.querySelector("#history-continue").checked =
        settings.continuePlayback;
    const codecStatus = document.querySelector("#native-hevc-status");
    codecStatus.textContent = isNativeHevcSupported
    ...
    codecStatus.className = isNativeHevcSupported
        ? "codec-available"
        : "codec-unavailable";
    renderCacheStats(payload.cache);
    setStatus(
        document.querySelector("#app-settings-state"),
        payload.persistent ? "Saved locally" : "Session defaults"
    );
    document.querySelector("#app-settings-status").textContent =
        payload.persistent
 ...
 *
 */
async function loadAppSettings(shouldForceFormRefresh = false) {
    const panel = document.querySelector("#panel-settings");
    panel.setAttribute("aria-busy", "true");
    try {
        const payload = await getJson("/api/settings");
        renderAppSettings(payload, shouldForceFormRefresh);
    } finally {
...
function readAppSettingsForm() {
    const cacheGibibytes =
        document.querySelector("#cache-size-gib").valueAsNumber;
    const cacheMaxBytes = Math.round(cacheGibibytes * 1024 ** 3);
    if (!Number.isSafeInteger(cacheMaxBytes))
        throw new Error("Cache size is outside the supported range.");
    return {
        cacheMaxBytes,
        cacheMaxEntries:
            document.querySelector("#cache-max-entries").valueAsNumber,
        continuePlayback: document.querySelector("#continue-playback-setting")
            .checked,
        previewVideoCodec: document.querySelector(
            "#preview-video-codec-setting"
        ).value,
        rewindPreviewSeconds: Number(
            document.querySelector("#rewind-window-setting").value
        ),
    };
}
...
function resolvedPreviewVideoCodec() {
    ...
    return isNativeHevcSupported ? "hevc" : "h264";
}
...
async function saveAppSettings() {
    ...
    try {
        ...
        setIconLabel(button, "", "Save settings");
    }
}
...
async function clearMediaCache() {
    ...
    try {
        ...
        button.disabled = false;
        setIconLabel(button, "", "Clear cache");
    }
}
...
function renderHistory(payload) {
    ...
    document.querySelector("#history-day-label").textContent = cameraDate(
    ...
    for (const [index, recording] of recordings.entries()) {
        const bounds = recordingBounds(recording);
        const start = (bounds.start / 86_400) * 100;
        const duration = bounds.end - bounds.start;
        const width = (duration / 86_400) * 100;
        ...
        segment.setAttribute(
            "aria-label",
            `${duration > 0 ? "Play" : "Unplayable"} recording ${index + 1}, ${beginTime} to ${endTime}`
        );
        segment.addEventListener("click", () => {
            selectHistorySegment(index, bounds.start);
        });
    ...
    setScrubberPosition(
        recordings.length > 0 ? recordingBounds(recordings[0]).start : 0
    );
}
...
async function loadHistoryOnce() {
    ...
    try {
        const payload = await getJson(
            `/api/history?date=${encodeURIComponent(dateInput.value)}`
        );
...
function previewScrubberMoment() {
    ...
    status.textContent =
        index >= 0
            ? `Release to open ${clockForSeconds(seconds)}.`
            : `No footage at ${clockForSeconds(seconds)}.`;
}
...
function selectScrubberMoment() {
    ...
    document.querySelector("#history-title").textContent =
        `${clockForSeconds(seconds)} · no recording`;
    document.querySelector("#history-position").textContent =
        `— / ${historyData?.recordings.length || 0}`;
    document.querySelector("#history-previous").disabled = true;
    document.querySelector("#history-next").disabled = true;
...
function selectHistorySegment(index, atSeconds = null) {
    if (historyPreviewOpening) {
        document.querySelector("#history-status").textContent =
            "A preview is already opening; wait for it to finish.";
        return;
    }
    const recordings = historyData?.recordings || [];
    ...
    const requestedSeconds =
        atSeconds === null
            ? bounds.start
            : Math.max(bounds.start, Math.min(bounds.end - 1, atSeconds));
    selectedHistoryIndex = index;
    selectedHistorySeconds = requestedSeconds;
    setScrubberPosition(requestedSeconds);
    highlightHistorySelection(index);
    document.querySelector("#history-title").textContent =
        `${clockForSeconds(requestedSeconds)} · ${historyWindowLabel(bounds.end - requestedSeconds)}`;
    document.querySelector("#history-position").textContent =
        `${index + 1} / ${recordings.length}`;
    document.querySelector("#history-previous").disabled = index <= 0;
    document.querySelector("#history-next").disabled =
...
async function buildHistoryPreview(recording, atSeconds) {
    ...
    if (historyWindow.value !== "full") {
        ...
        estimatedBytes =
            recording.sizeBytes *
            (Math.min(duration, bounds.end - atSeconds) / recordingDuration);
    }
    historyPlaybackStartSeconds = atSeconds;
    card.setAttribute("aria-busy", "true");
    const startedAt = performance.now();
    let isPlaybackStarted = false;
    let isUsedCompatibleFallback = false;
    ...
    try {
        try {
            ...
            const preference =
                appSettingsData?.settings.previewVideoCodec || "auto";
            if (
                ...
                throw error;
            isUsedCompatibleFallback = true;
            stopMediaLoad(historyVideo);
            parameters.set("videoCodec", "h264");
            previewUrl = `/api/history/preview?${parameters}`;
            status.textContent =
                "Native HEVC did not play; retrying compatible H.264…";
            cacheState = await previewCacheState(previewUrl, request.signal);
            await loadVideoStream(historyVideo, previewUrl, request.signal);
        }
        ...
        await historyVideo.play().catch(() => undefined);
        status.textContent = `${isUsedCompatibleFallback ? "Compatible H.264 fallback · " : ""}Ready in ${loadTimeLabel(startedAt)} · cached when complete.`;
        if (cacheState === "missing") {
            void promoteCachedVideo(
                ...
                () => {
                    status.textContent =
                        "Indexed cache ready · full seek bar and audio available.";
                }
            ).catch((error) => {
                if (
        ...
        if (historyRequest === request) {
            if (!isPlaybackStarted) historyRequest = null;
...
function fillTimelapseForm(config) {
    ...
    document.querySelector("#timelapse-interval-seconds").value =
 ...
 *
 */
function renderTimelapse(payload, shouldForceFormRefresh) {
    ...
    document.querySelector("#timelapse-progress-label").textContent =
        `${config.progressPercent.toFixed(1)}% complete`;
    document.querySelector("#timelapse-captures").textContent =
        `~${config.estimatedCaptures.toLocaleString()} of ${config.expectedCaptures.toLocaleString()} captures`;
    document.querySelector("#timelapse-dates").textContent =
        `${displayDate(config.startTime)} → ${displayDate(config.endTime)}`;
    document.querySelector("#timelapse-interval").textContent =
        `One frame every ${formatInterval(config.intervalSeconds)} · daily ${config.dailyStart}–${config.dailyEnd}`;
    if (!isFormIsDirty || shouldForceFormRefresh) fillTimelapseForm(config);
...
function renderTimelapseFiles(recordings) {
    ...
    document.querySelector("#timelapse-file-status").textContent =
        `${recordings.length} time-lapse file${recordings.length === 1 ? "" : "s"}.`;
}

/**
 *
 */
function loadTimelapse(shouldForceFormRefresh = false) {
    if (timelapseLoadPromise) return timelapseLoadPromise;
    timelapseLoadPromise = loadTimelapseOnce(shouldForceFormRefresh).finally(() => {
        timelapseLoadPromise = null;
    });
 ...
 *
 */
async function loadTimelapseOnce(shouldForceFormRefresh) {
    ...
    try {
        ...
        isTimelapseLoaded = true;
        renderTimelapse(payload, shouldForceFormRefresh);
    } finally {
...
function visibleCameraFiles() {
    ...
    const files = [...(filesData?.files || [])].filter((file) => {
        const isMatchesKind = kind === "all" || file.kind === kind;
        const haystack =
            `${file.fileName} ${file.downloadName} ${file.beginTime} ${file.endTime} ${file.kind}`.toLocaleLowerCase();
        return isMatchesKind && (!query || haystack.includes(query));
    });
    files.sort((left, right) => {
        if (sort === "oldest")
            return cameraDate(left.beginTime) - cameraDate(right.beginTime);
...
async function previewCameraFile(file) {
    ...
    try {
        if (file.kind === "timelapse") {
            if (!isTimelapseLoaded) await loadTimelapse();
        ...
        await loadHistory();
        const index =
            historyData?.recordings.findIndex(
                (item) => item.fileName === file.fileName
            ) ?? -1;
        if (index < 0)
            throw new Error(
                "This recording is no longer present in the camera index."
            );
        activateTab("rewind", false, true);
        selectHistorySegment(
            index,
            recordingBounds(historyData.recordings[index]).start
        );
    } catch (error) {
        status.textContent = errorMessage(error);
    }
...
function renderFiles() {
    ...
    for (const file of files) {
        ...
        durationCell.textContent = formatFileDuration(
            (cameraDate(file.endTime) - cameraDate(file.beginTime)) / 1000
        );
        const sizeCell = document.createElement("td");
        ...
        if (file.downloadable) {
            ...
            setIconLabel(download, "", "Download");
            actionCell.append(download);
        }
        row.append(
            typeCell,
    ...
    document.querySelector("#files-count").textContent =
        `${files.length} file${files.length === 1 ? "" : "s"}`;
    document.querySelector("#files-breakdown").textContent =
        `${recordings} recordings · ${timelapses} time-lapses`;
    document.querySelector("#files-size").textContent = formatSize(
        files.reduce((total, file) => total + file.sizeBytes, 0)
    );
    status.hidden = files.length > 0;
    status.textContent = filesData?.files.length
...
function loadFiles() {
    ...
    filesLoadPromise = loadFilesOnce().finally(() => {
...
async function loadFilesOnce() {
    ...
    try {
        filesData = await getJson(
            `/api/files?date=${encodeURIComponent(dateInput.value)}`
        );
        isFilesLoaded = true;
        renderFiles();
    } finally {
...
function readTimelapseForm() {
    const config = {
        ...
        intervalSeconds: document.querySelector("#timelapse-interval-seconds")
            .valueAsNumber,
        startTime: document.querySelector("#timelapse-start").value,
    };
    if (new Date(config.endTime) <= new Date(config.startTime))
        throw new Error("End must be after start.");
    if (config.dailyEnd <= config.dailyStart)
        throw new Error("Daily end must be after daily start.");
    return config;
}
...
function showChangeReview(config) {
    ...
    addSummaryItem(
        summary,
        "Daily window",
        `${config.dailyStart} → ${config.dailyEnd}`
    );
...
async function applySettings() {
    ...
    const submittedConfig = pendingConfig;
    ...
    try {
        const response = await fetch("/api/timelapse", {
            body: JSON.stringify({
                ...
                expectedRevision: timelapseData.config.revision,
            }),
            headers: {
                "Content-Type": "application/json",
                "X-GrowCam-Request": "1",
        ...
        if (!response.ok) {
            ...
            const rollback =
                payload.rollbackVerified === true
                    ? " The previous schedule was restored."
                    : "";
            throw new Error(`${failure}${rollback}`);
        }
        confirmDialog.close();
        if (pendingConfig === submittedConfig) {
            pendingConfig = null;
            isFormIsDirty = false;
        }
        ...
        button.disabled = false;
        setIconLabel(button, "", "Apply to camera");
    }
}
...
async function buildPreview(recording) {
    ...
    document.querySelector("#preview-title").textContent = recording.active
        ? "Opening progress"
        : "Opening preview";
    ...
    try {
        ...
        if (cacheState === "missing") {
            void promoteCachedVideo(
                ...
                () => {
                    status.textContent =
                        "Complete 25 fps preview cached · smooth replay and seeking ready.";
                }
            ).catch((error) => {
                if (
                    (!(error instanceof DOMException) ||
        ...
        if (previewRequest === request) {
            if (!isPlaybackStarted) previewRequest = null;
...
async function refresh() {
    ...
    if (settingsResult.status === "rejected") {
        ...
        document.querySelector("#app-settings-status").textContent =
            errorMessage(settingsResult.reason);
    }
...
async function retryCameraControls() {
    ...
    setIconLabel(cameraControlRetry, "", "Trying one login…");
    ...
    try {
        ...
        const activeTabName = activeTab?.dataset.tab || "live";
        if (cameraControlTabs.has(activeTabName))
            await ensureTabData(activeTabName);
    } catch (error) {
        renderCameraControlUnavailable(error);
        setStatus(connection, "Live feed only", "warning");
    } finally {
...
document.querySelector("#history-previous").addEventListener("click", () => {
    ...
    if (recording)
        selectHistorySegment(index, recordingBounds(recording).start);
});
...
historyWindow.addEventListener("change", () => {
    if (selectedHistoryIndex >= 0 && selectedHistorySeconds !== null)
        selectHistorySegment(selectedHistoryIndex, selectedHistorySeconds);
});
...
historyVideo.addEventListener("ended", () => {
    if (
        document.querySelector("#panel-rewind").hidden ||
        !document.querySelector("#history-continue").checked ||
    ...
    if (historyWindow.value === "full") {
        ...
        if (recording)
            selectHistorySegment(nextIndex, recordingBounds(recording).start);
        return;
    }
    const selectedRecording = recordings[selectedHistoryIndex];
    if (!selectedRecording) return;
    const nextSeconds = Math.min(
        historyPlaybackStartSeconds + Number(historyWindow.value),
        recordingBounds(selectedRecording).end
    );
...
document
    .querySelector("#live-pause-toggle")
    .addEventListener("click", toggleLiveFeed);
document
    .querySelector("#live-audio-toggle")
    .addEventListener("click", () => void toggleLiveAudio());
document
    .querySelector("#show-clear-cache")
    .addEventListener("click", () => cacheClearDialog.showModal());
document
    .querySelector("#clear-cache")
    .addEventListener("click", () => void clearMediaCache());
document
    .querySelector("#refresh-timelapse")
...
previewVideo.addEventListener("ended", () => {
    ...
    const decodedFrames =
        previewVideo.getVideoPlaybackQuality?.().totalVideoFrames;
    let frameCount = "";
    ...
    document.querySelector("#preview-status").textContent =
        `${frameCount}${formatDuration(previewVideo.duration)} compressed video covering ${displayDate(currentPreviewRecording.beginTime)} → ${displayDate(currentPreviewRecording.endTime)} · complete preview cached locally.`;
});
...
for (const button of liveQualityButtons) {
    button.addEventListener("click", () =>
        { selectLiveQuality(button.dataset.liveQuality); }
    );
}
...
liveFeed.addEventListener("load", () => {
    ...
    const resolution =
        liveFeed.naturalWidth && liveFeed.naturalHeight
            ...
            : "connected";
    document.querySelector("#live-resolution").textContent =
        `${liveQuality.toUpperCase()} · ${resolution}`;
...
liveFeed.addEventListener("error", () => {
    ...
    liveMessage.textContent =
        "Live feed unavailable. Check the camera connection and FFmpeg.";
    livePlaceholder.hidden = false;
    updateLiveQualityButtons(false);
});
liveAudio.addEventListener("playing", () =>
    { updateLiveAudioState(true, "audio live"); }
);
